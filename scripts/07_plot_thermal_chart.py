...
GEE Triple-Satellite (L7+L8+L9) Paired BACI LST Analysis
三面板：STL趋势 + 全年BACI + 夏季专项BACI
"""
...
def render_thermodynamic_chart(csv_path, output_image_path):
    ...
    ax1.plot(sprawl_trend.index, sprawl_trend, color='#FF8C00', linewidth=3,
             label='Impact Zone (Parking Lot) — STL Trend')
    ...
    ax1.set_title('Paired BACI Thermal Audit: Single Parking Lot (L7+L8+L9)',
    ...
    ax3.scatter(s_pre.index, s_pre.values, color='#66CCFF', alpha=0.7, s=40, marker='D', zorder=3,
                ...
                label=f'Pre Summer ΔT (n={len(s_pre)}, μ={s_pre.mean():+.2f}°C)')
    ax3.scatter(s_post.index, s_post.values, color='#FF4444', alpha=0.7, s=40, marker='D', zorder=3,
                edgecolors='white', linewidth=0.5,
                label=f'Post Summer ΔT (n={len(s_post)}, μ={s_post.mean():+.2f}°C)')
    ...
    sj_label = (f"Summer BACI (JJA): Welch p={s_t_p:.3e} ({_sig_label(s_t_p)}), "
    ...
    ax3.set_ylabel(r'$\Delta$T Summer JJA (°C)', fontsize=12, fontname='Courier New', color='#FF8888')
